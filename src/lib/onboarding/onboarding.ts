...
function detectBaseDomainFromWildcardBlock(content: string): string | null {
  const wildcard = content.match(/\*\.([A-Za-z0-9.-]+)\s*,\s*([A-Za-z0-9.-]+)\s*\{/);
...
function detectDirectivesFromWildcardBlock(content: string): string {
  const wildcardHeader = content.search(/\*\.[A-Za-z0-9.-]+\s*,\s*[A-Za-z0-9.-]+\s*\{/);
  ...
  const preRoutes = firstRouteIndex >= 0 ? body.slice(0, firstRouteIndex) : body;
...
function detectDashboardUpstreamFromWildcardBlock(content: string): string {
  ...
  const matches = [...body.matchAll(/handle\s*\{\s*[\s\S]*?reverse_proxy\s+([^\s}]+)/gm)];
...
function toManualCommands(caddyApiUrl: string, caddyfilePath: string): string[] {
  return [
...
export async function ensureOnboardingDraft(): Promise<OnboardingDraft | null> {
  ...
  const parsedSites = caddyfile ? uniqueSites(parseSitesFromCaddy(caddyfile)) : [];
...
export async function runStartupBootstrap(): Promise<void> {
  ...
  if (!result.ok) {
    console.warn(`[startup] Caddy API unavailable; continuing in degraded mode: ${result.error}`);
  }
...
export async function completeOnboarding(input: {
  ...
  if (!siteBlockDirectives) {
    return { ok: false, error: 'Site config directives are required.', manualCommands: [] };
  }
  if (!caddyApi) {
    return { ok: false, error: 'Caddy API URL is required.', manualCommands: [] };
  }
  if (!dashboardUpstream) {
    return { ok: false, error: 'Dashboard upstream is required.', manualCommands: [] };
  }

  await markOnboardingCompleted(baseDomain, caddyApi, dashboardUpstream, siteBlockDirectives);
