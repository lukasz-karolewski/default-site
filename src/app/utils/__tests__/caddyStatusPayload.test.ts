...
vi.mock('~/app/utils/runtimePaths', () => ({
...
describe('buildCaddyStatusPayload', () => {
  beforeEach(() => {
    ...
    mockSnapshot.mockResolvedValue({
      ...
      lastManagedWriteHash: '5c6c35d12723f2003e84b567446ebfbabfe8ff36a258a00b0985d62abddce1e3',
    });
  });

  it('reports unchanged when file hash matches last managed write hash', async () => {
    mockStat.mockResolvedValue({ mtime: new Date('2026-02-26T00:01:00.000Z'), size: 123 } as never);
  ...
  it('reports changed when file hash differs from last managed write hash', async () => {
    mockStat.mockResolvedValue({ mtime: new Date('2026-02-26T00:02:00.000Z'), size: 321 } as never);
